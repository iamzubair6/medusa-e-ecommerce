...
| 10 | 2026-07-16 | Admin used browser `confirm()`/`alert()` (jarring, unstyled, breaks the design system) | Never use `window.confirm`/`alert` in the app — use `ConfirmDialog` from `@ecom/ui` for confirmations and `useToast` for feedback |
